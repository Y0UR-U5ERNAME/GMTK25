...
func _ready() -> void:
	...
	if GlobalVariables.faulty_rate == 0:
		...
		moneys.text = "(+$%d)\n(-$%d)\n(+$%d)\n(-$%d)" % GlobalVariables.stats.slice(1, 8, 2)
...
func _process(delta: float) -> void:
	...
	if 1 < progress and progress < (6 if GlobalVariables.faulty_rate == 0 else 10):
		categories.max_lines_visible = progress / 2
		values.max_lines_visible = (progress - 1) / 2
		moneys.max_lines_visible = (progress - 1) / 2
	
	if progress >= (6 if GlobalVariables.faulty_rate == 0 else 10): categories.max_lines_visible = -1
	if progress >= (7 if GlobalVariables.faulty_rate == 0 else 11):
		values.max_lines_visible = -1
		moneys.max_lines_visible = -1
	
	if progress >= (8 if GlobalVariables.faulty_rate == 0 else 12):
